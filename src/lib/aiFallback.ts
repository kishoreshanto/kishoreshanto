...
const KNOWLEDGE_BASE: KnowledgeItem[] = Object.entries(KNOWLEDGE_BASE_RAW)
...
class OfflineSearchEngine {
  ...
  public search(query: string, scoreThreshold = 1): string {
    ...
    for (const item of this.processedKnowledge) {
      ...
      if (item.source.includes('personal') && (queryText.includes('who') || queryText.includes('about'))) {
        currentScore += 1;
      }
      if (item.source.includes('research') && (queryText.includes('research') || queryText.includes('work'))) {
        currentScore += 1;
      }
      if (item.source.includes('contact') && (queryText.includes('contact') || queryText.includes('reach'))) {
        currentScore += 1;
      }
