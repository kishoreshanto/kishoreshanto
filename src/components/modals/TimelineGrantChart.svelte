	...
	function arrangeProjectsInRows() {
		...
		for (const projectData of sortedProjects) {
			...
			for (const row of rows) {
				const canPlace = row.projects.every(existing => 
...
<div class="timeline-chart-container bg-white dark:bg-gray-900 p-6 rounded-lg max-h-[80vh] overflow-hidden">
	<!-- Timeline Chart -->
	<div class="timeline-chart mb-8">
		<div 
			...
			style="height: 400px;"
		>
			<div class="timeline-content relative" style="width: {totalMonths * monthWidth}px; height: 100%;">
				<!-- Header: Years -->
							...
							{#each row.projects as projectData}
								<div 
									...
									style="
										...
										top: 5px;
									"
									title="{projectData.project.title} ({projectData.startMonth}/{projectData.startYear} - {projectData.endMonth}/{projectData.endYear})"
								>
	...
	<div class="legend">
		...
		<div class="legend-grid grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 max-h-48 overflow-y-auto">
			{#each projects as project}
				<div class="legend-item flex items-center gap-3 p-2 rounded hover:bg-gray-50 dark:hover:bg-gray-800">
					<div 
						class="legend-color w-4 h-4 rounded flex-shrink-0"
						style="background-color: {project.color};"
					></div>
					<span class="legend-text text-sm text-gray-800 dark:text-gray-200 truncate" title="{project.title}">
						{project.title}
					</span>
				</div>
			{/each}
