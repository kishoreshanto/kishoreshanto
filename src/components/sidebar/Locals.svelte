...
<script lang="ts">
    ...
    const current_gmt_offset: string | undefined = data.current_gmt_offset;

    let offsetNumber: number;
    let LT_GMT: { local_time: string, GMT: string };
    if (current_gmt_offset) {
        offsetNumber = parseFloat(current_gmt_offset);
        LT_GMT = getLocalTimeFromUTCOffset(offsetNumber);
    }
</script>
    ...
    <div class="mt-4 flex gap-8 select-none cursor-default">
        {#if current_city}
            <div class="flex  items-center">
                <svg class="fill-zinc-500 dark:fill-zinc-400 mr-2" xmlns="http://www.w3.org/2000/svg" height="24px"
        ...
        {#if current_gmt_offset}
            <div class="flex items-center">
                <svg class="fill-zinc-500 dark:fill-zinc-400 -ml-0.5 mr-2 h-5" xmlns="http://www.w3.org/2000/svg"
                     height="24px"
